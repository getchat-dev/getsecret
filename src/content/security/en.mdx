---
title: "Security"
lede: "Passwords, tokens, and keys fly through work chats every day. Getsecret lets you hand over a secret once — without leaving a trail in your message history."
metaDescription: "Why a password pasted into chat stays there for years, how Getsecret encrypts it in your browser instead, and what it still cannot protect you from."
---

## The problem

Someone drops a production key into Slack. Someone sends an admin password over Telegram "for five minutes." Someone notes it down to themselves so they won't forget.
...
- IBM estimates the average cost of a single corporate breach at $4.88M.
...
2. Your browser then uses that key to encrypt your text with AES-256-GCM. Only the encrypted string travels to the server — without the key, it can't be read.
...
<code className="prose-example">https://getsecret.app/s/abc123#xyz789</code>
...
Getsecret removes one specific risk — the trail in your message history. No more, no less.

For the full picture — what we protect, who we protect you from, and where we explicitly stop — read the [threat model](/threat-model).
